...
class PetsupplierController extends GetxController {
  ...
  void fetchSuppliers() async {
    try {
      isLoading(true);
      final response = await http.get(Uri.parse(ApiLinkNames.allsuppliers));
      print(response.statusCode);
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body) as List;
        petDoctors.value = data.map((json) => Doctor.fromJson(json)).toList();
      } else {
        Get.snackbar('Error', 'Failed to load suppliers doctors');
      }
    } finally {
      isLoading(false);
    }
